...
Production deployment is intentionally isolated from ordinary development branches. Before the production build, migrations, or deployment, the workflow authenticates to Cloudflare, reads the currently deployed Worker's D1 binding, and hydrates the local deployment config only inside the ephemeral CI workspace.
