    ...
    <section className="reader-pane">
      <header className="reader-toolbar">
        ...
        <div className="reader-actions">
          <button type="button" onClick={onReply}><Icon name="reply" size={16} /><span>Reply</span></button>
          <button type="button" onClick={() => onPatch({ isStarred: !message.isStarred })}><Icon name="star" size={16} /><span>{message.isStarred ? "Unstar" : "Star"}</span></button>
          {!message.isDeleted ? (
            <button type="button" onClick={() => onPatch({ isArchived: !message.isArchived }, true)}><Icon name="archive" size={16} /><span>{message.isArchived ? "Unarchive" : "Archive"}</span></button>
          ) : null}
          {!message.isDeleted ? (
            <button className="destructive" type="button" onClick={() => onPatch({ isDeleted: true }, true)}><Icon name="trash" size={16} /><span>Trash</span></button>
          ) : (
            <button type="button" onClick={() => onPatch({ isDeleted: false }, true)}><Icon name="inbox" size={16} /><span>Restore</span></button>
          )}
        </div>
      </header>
      ...
      <article className="reader-scroll">
        ...
        <div className="message-content">
          ...
          {message.bodyHtmlAvailable ? (
            <div className="security-note"><Icon name="lock" size={14} /><span>HTML is shown as safe text. Remote images stay blocked.</span></div>
          ) : null}
        </div>

        {message.attachments.length > 0 ? (
          <section className="reader-attachments" aria-label="Attachments">
            <h3>Attachments <span>{message.attachments.length}</span></h3>
            <div className="attachment-grid">
              {message.attachments.map((attachment) => (
                <a key={attachment.id} className="attachment-card" href={`/api/attachments/${encodeURIComponent(attachment.id)}`}>
                  <span className="attachment-card-icon"><Icon name="paperclip" size={17} /></span>
      ...
      <footer className="reader-reply-bar">
        <button type="button" onClick={onReply}><Icon name="reply" size={17} />Reply</button>
      </footer>
    </section>
  );
}
